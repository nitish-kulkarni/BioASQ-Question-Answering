...
def test(val_data, val_outputs, net):
    ...
    correct_pred = 0
    total_pred = 0
    tot_batches = val_data.shape[0] // args.test_batch_size
    count = 0
    for batch_id in tqdm(range(0, tot_batches)):
	if batch_id != tot_batches:
		    x_batch = val_data[batch_id * args.test_batch_size: (batch_id + 1) * args.test_batch_size, ]
		    y_batch = val_outputs[batch_id * args.test_batch_size: (batch_id + 1) * args.test_batch_size, ]
        else:
		    x_batch = val_data[batch_id * args.test_batch_size:, ]
		    y_batch = val_outputs[batch_id * args.test_batch_size:, ]

        x_batch = torch.autograd.Variable(torch.FloatTensor(x_batch), volatile=True)
        y_batch = torch.autograd.Variable(torch.FloatTensor(y_batch), volatile=True)
        if use_cuda:
...
class Net(nn.Module):
    def __init__(self, bio_embedding_dim = 200, glove_embedding_dim = 300):
...
def train(train_data, train_outputs):
	net = Net(train_data.shape[1], train_outputs.shape[1])
	if use_cuda:
	...
	for epoch in range(args.epochs):
	    ...
	    #for batch_id in tqdm(range(0, tot_batches)):
            for batch_id in range(0, tot_batches):
	        if batch_id != tot_batches:
	      	...
	      	y_batch = torch.autograd.Variable(torch.FloatTensor(y_batch))
	    ...
	    if (epoch+1) % 10 == 0:
	      ...
	      print('Epoch: '+str(epoch)+': Test Loss is: '+str(val_loss))
              #pdb.set_trace()
